...
import com.example.screenshotapp.ScreenshotApplication
...
class ScreenshotForegroundService : Service() {
    ...
    companion object {
        const val ACTION_START_CAPTURE = "com.example.screenshotapp.START_CAPTURE"
        const val ACTION_STOP_CAPTURE = "com.example.screenshotapp.STOP_CAPTURE"
        const val ACTION_CAPTURE_NORMAL = "com.example.screenshotapp.CAPTURE_NORMAL"
        const val ACTION_CAPTURE_LONG = "com.example.screenshotapp.CAPTURE_LONG"
    ...
    private fun createNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, ScreenshotApplication.NOTIFICATION_CHANNEL_ID)
    ...
    private fun saveScreenshot(bitmap: Bitmap, mode: ScreenshotMode) {
        serviceScope.launch {
            val storageManager = com.example.screenshotapp.data.storage.FileStorageManager(this@ScreenshotForegroundService)
            val record = storageManager.saveScreenshot(bitmap, mode)
            
            if (record != null) {
                val entity = com.example.screenshotapp.data.database.ScreenshotEntity(
                    fileName = record.fileName,
                    filePath = record.filePath,
                    mode = com.example.screenshotapp.data.database.ScreenshotEntity.fromScreenshotMode(record.mode),
                    timestamp = record.timestamp,
                    pixelValue = record.pixelValue,
                    algorithm = record.algorithm?.let { 
                        com.example.screenshotapp.data.database.ScreenshotEntity.fromStitchAlgorithm(it) 
                    },
                ...
                ScreenshotApplication.getDatabase(this@ScreenshotForegroundService)
                    .screenshotDao()
                    .insert(entity)
            }
        }
